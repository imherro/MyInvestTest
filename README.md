...
程序默认只绘制 `480092.CNI` 国证自由现金流全收益指数，并在收盘价曲线上标记高低交替的波段拐点。默认用 `15%` 反向幅度确认拐点，可通过 `--turning-reversal` 调整：

- 主曲线：`480092.CNI` 收盘价
- 拐点折线：按时间连接起点、有效高拐点、有效低拐点和末段极值
- 有效高拐点：上升段持续更新最高点，从最高点回撤达到阈值后确认
- 有效低拐点：下降段持续更新最低点，从最低点反弹达到阈值后确认
- 默认阈值：`0.15`，即反向移动 `15%` 才确认上一段拐点
